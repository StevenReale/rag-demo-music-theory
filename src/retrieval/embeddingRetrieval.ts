...
import { Chunk, EmbeddedChunk, ScoredChunk } from "@/library/types"
import { embedTexts } from "@/openai/embeddings";
import { EMBEDDINGS_CACHE_PATH, EMBEDDING_MODEL_NAME, MIN_CHARS_FOR_RETRIEVAL } from "@/appConfig";
...
export function searchEmbeddedChunks(
    ...
    for (const {chunk, embedding} of embeddedChunks) {
        if (chunk.text.trim().length < MIN_CHARS_FOR_RETRIEVAL) continue;
